...
file_name = "analysis_config/test_contours_decay.ini"
        ...
        if ((offset + length) > 0) and (sim_length > offset):
            ...
            try:
                ...
                if fits is not None:
                    ...
                    log_sens_map[i][j] = np.log10(fits["polynom"])
            except ValueError:
                pass
            # except KeyError:
            #     pass
...
vrange = [
    [np.nanmin(sens_map), 3 * np.nanmin(sens_map)],
    [np.nanmin(log_sens_map), np.nanmax(log_sens_map)]
]
...
for i, map_to_plot in enumerate([sens_map, log_sens_map]):
    ...
    plt.imshow(
        ...
        vmin=vrange[i][0], vmax=vrange[i][1],
        )
    cbar = plt.colorbar()
    ...
    plt.scatter(x=offsets[loc[1]], y=lengths[loc[0]], marker="*",
                color="white",
                )

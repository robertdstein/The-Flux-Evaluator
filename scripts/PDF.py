...
#=====================================================================================
	...
	def GenerateSplineStuff(self, exp, mc, gamma=2.):
		"""Produces normailsed histograms for signal and background. Produces a histogram of the ratio
		of sig/bkg. Interpolates, with a spline function, of the log of this 2D distribution.
		"""
		...
		#Bitwise Addition giving an Array with "True" if "True" in both Sig and Bkg, otherwise False
		...
		#Returns Bin centers, and order of spline (for x and y)
		sinDecBinCenter = (self.sinDecBins[:-1]+self.sinDecBins[1:])/2.
		logEBinCenter = (self.EnergyBins[:-1]+self.EnergyBins[1:])/2.
		logE_order = 2

		#Fits a 2D spline function to the log of ratio array (2nd order in both dimensions)

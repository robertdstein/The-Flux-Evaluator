...
class sensitivity():
	...
	def __init__(self, path='test_stat_results/test_setup.pkl', plot_path='plots/LikelihoodLandscape/test_setup',
				 plotting=False, UpperLimit=False):
		self.path = path
	...
	def FindTestStatThreshold(self, ):
		...
		if False:
			...
			plt.figure()
			plt.hist(data, bins=50, lw=2, histtype='step', normed=True, color='black', label='BG Test Stat')
		...
		if self.alpha>0.1:
			self.TestStatThreshold = np.percentile(self.all_data['test_stat_sorted_all'][0.0], (1.-self.alpha)*1.e2)
		else:
			...
			if True:                
				...
				plt.axhline(self.alpha, color=col[1], lw=1, label=r'Significance')
				plt.axvline(AlphaSigma, lw=1, color=col[1])
				plt.hist(data, bins=100, lw=2, histtype='step', normed=True, color='black', label='BG Test Stat', cumulative=-1)
				plt.plot(x, y, lw=2, color=col[15], label=r'$\chi^2$ fit, df={0:6.2f}'.format(fit_res[0]))
				plt.semilogy()
				plt.xlim(0., 3.e1)
				plt.ylim(1.e-7, 1.e0)
				plt.xlabel(r"$\lambda$")
				plt.ylabel(r"$\int_{\lambda}^\infty P(\lambda^\prime)\mathrm{d}\lambda^\prime$")
				plt.legend(loc='best', fancybox=True, framealpha=1.)
	...
	def SensInterpoaltionFunction(self, x, a):
		value = (1. - np.exp(-np.log10(a) * x))*(1.-self.alpha) + self.alpha
		return value
	...
	def FindSensitivity(self, ):
		...
		f3 = lambda x: self.SensInterpoaltionFunction(x, self.DetChanceMyFit) - self.beta
		...
		if self.plotting:
			...
			plt.plot(x, self.SensInterpoaltionFunction(x, self.DetChanceMyFit),
					 lw=2, color='black', label=r'$(1-\exp(-ax))+{0:6.2f}$'.format(1.-self.alpha))           
...
#            plt.grid()
		...
		try:
			print('Interpolation: ', brentq(f2, FluxScaleMin, FluxScaleMax))
		except:
			print ('Interpolation: Failed')
		try:
			print('My Interpolation: ', brentq(f3, FluxScaleMin, FluxScaleMax))
		except:
	...
	def ComputeP_Value(self, ):
		...
		print(('p-value: ' + str(100.-stats.percentileofscore(x, MeasuredLambda, kind='weak')) + '%'))
		print(('p-value: ' + str(100.-stats.percentileofscore(x, MeasuredLambda, kind='strict')) + '%'))
	...
	def pValueFunction(self, MeasuredLambda):
		self.GenerateDistributions()
		x = np.sort(self.test_stat_results[0])
		CorrectPValue = 100.-stats.percentileofscore(x, MeasuredLambda, kind='strict')
		...
		if MeasuredLambda==0.:
			...
			FitPValue = scp.stats.chi2.pdf(MeasuredLambda, fit_res[0]) * fraction * 100.

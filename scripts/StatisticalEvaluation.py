...
class sensitivity():
	...
	def GenerateDistributions(self, ):
		...
		for k in self.test_stat_results.keys():
			...
			self.all_data['cumu_dist_normed'][k] = np.cumsum(weights) / \
				np.sum(weights)
	...
	def FindTestStatThreshold(self, ):
		...
		if self.alpha > 0.1:
			...
			if True:
				...
				plt.axhline(self.alpha, color=col[1],
					lw=1, label=r'Significance')
				plt.axvline(AlphaSigma, lw=1, color=col[1])
				plt.hist(data, bins=100, lw=2, histtype='step', normed=True,
	...
	def FindSensitivity(self, ):
		...
		f3 = lambda x: self.SensInterpoaltionFunction(x,
			self.DetChanceMyFit) - self.beta
...
#            plt.grid()
			...
			plt.ylim(.2, 1.)
			try:
				plt.axvline(x=brentq(f, FluxScaleMin, FluxScaleMax),
					lw=2, color='red')
			except:
				pass
			try:
				plt.axvline(x=brentq(f2, FluxScaleMin, FluxScaleMax),
					lw=2, color='blue')
			except:
				pass
			try:
				plt.axvline(x=brentq(f3, FluxScaleMin, FluxScaleMax),
					lw=2, color='black')
			except:
				pass
			plt.legend(loc='best', fancybox=True, framealpha=1.)

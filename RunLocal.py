...
if cfg.config not in conf._sections:
	...
	cat_dir = "/afs/ifh.de/user/a/astasik/scratch/PS_Data/"
	...
	settings = {'UseEnergy': eval(conf.get(cfg.config, "UseEnergy")),
					...
					'TimeBoxLenght': float(conf.get(cfg.config, "TimeBoxLength")),
					'UseDecayModel': eval(conf.get(cfg.config, "UseDecayModel")),
					'Model_tpp': float(conf.get(cfg.config, "Model_tpp")),
					'DecayModelLenght': eval(conf.get(cfg.config, "DecayModelLength")),
